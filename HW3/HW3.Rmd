...
icustays_tble %>% 
  group_by(subject_id) %>% 
  filter(intime == min(intime)) %>% #Finds first admission per subject ID
  ungroup() %>% 
  left_join(admissions_tble) %>%
  left_join(patients_tble) %>% #merge in admissions and patients
  left_join(labevents_tble, 
            by = c("subject_id", "hadm_id")) %>%
  left_join(chartevents_tble, 
            by = c("subject_id", "hadm_id", "stay_id")) %>%
  mutate(tbl_age = anchor_age + year(admittime) - anchor_year) %>% #merge in labevents and chartevents
  filter(tbl_age >= 18) %>% #adults only
  show_query() %>% #display SQL Query
  collect() %>% #collect as a tibble
  mutate(death30 = deathtime - admittime) %>%
  mutate(death30 = case_when(death30 <= 30*24*3600 ~ "yes",
                             death30 > 30*24*3600 ~ "no",
                             TRUE ~ "no")) %>%
